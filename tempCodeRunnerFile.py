...
class ConsultaWindow(tk.Tk):
    ...
    def create_widgets(self):
        filter_frame = tk.Frame(self, bg="#add8e6")
        filter_frame.pack(pady=10)

        label_style = ("Century Gothic bold", 12)

        lb_filter_placa = tk.Label(filter_frame, text="Filtrar por Placa:", font=label_style, bg="#add8e6")
        lb_filter_placa.grid(row=0, column=0, padx=5)

        self.filter_placa_var = tk.StringVar()
        entry_filter_placa = tk.Entry(filter_frame, textvariable=self.filter_placa_var, font=label_style)
        entry_filter_placa.grid(row=0, column=1, padx=5)

        btn_consulta_placa = tk.Button(filter_frame, text="Consultar Placa", command=self.consultar_por_placa, bg="#add8e6", relief=tk.FLAT)
        btn_consulta_placa.grid(row=0, column=2, padx=5)

        lb_filter_nome = tk.Label(filter_frame, text="Filtrar por Nome:", font=label_style, bg="#add8e6")
        lb_filter_nome.grid(row=1, column=0, padx=5)

        self.filter_nome_var = tk.StringVar()
        entry_filter_nome = tk.Entry(filter_frame, textvariable=self.filter_nome_var, font=label_style)
        entry_filter_nome.grid(row=1, column=1, padx=5)

        btn_consulta_nome = tk.Button(filter_frame, text="Consultar Nome", command=self.consultar_por_nome, bg="#add8e6", relief=tk.FLAT)
        btn_consulta_nome.grid(row=1, column=2, padx=5)
        ...
        self.no_results_label = tk.Label(self, text="Nenhum resultado encontrado", font=label_style, bg="#add8e6", fg="red")
